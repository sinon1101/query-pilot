 ...
 *
 * @param round  ReAct 第几轮（从 1 开始）；route 步骤在进循环前产生，记为第 0 轮
 * @param type   thought（模型文本）/ tool_call（工具调用）/ guardrail（执行器注入的规则提醒）/
 *               reflection（Critic 语义审校结论）/ route（进循环前的复杂度分档决策）
...
public record AgentStep(int round, String type, String name, String input, String output, boolean error) {
    ...
    /** 复杂度分档决策：进 ReAct 循环前由 QuestionRouter 产生（记为第 0 轮），output 记档位与理由 */
    public static AgentStep route(String text) {
        return new AgentStep(0, "route", null, null, text, false);
    }
}
