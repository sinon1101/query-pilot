...
- [x] **反思开关 A/B 对照**（eval/questions-ab.txt 6 道语义陷阱题，真值全部手工核验；
  ...
  - ✅ **已复跑坐实**（2026-07-09，详见第六阶段末）：上表 AB 与原 20 题回归原是拿收紧前旧提示词跑的；
    收紧 + 路由后复跑原 20 题，那两个被 Critic 过度打回→超轮数的 case（城市Top5/周末工作日）
    从**撞满10轮失败变为1次成功**，任务完成率 90%→100%，平均耗时 46.3s→39.6s
...
- [x] **全量评测复跑坐实**（2026-07-09，三个评测集共 46 题，qwen3.7-plus + 路由 ON + 收紧后 Critic）：

  **原 20 题（与 phase-5 基线苹果对苹果比）：**

  | 指标 | phase-5 基线（路由OFF+收紧前） | 本次（路由ON+收紧后） | 变化 |
  |---|---|---|---|
  | 任务完成率 | 18/20 = 90% | **20/20 = 100%** | **+10pp** |
  | SQL 一次成功率 | 18/20 = 90% | 17/20 = 85% | -5pp（随机波动，自愈后仍100%） |
  | SQL 自愈后成功率 | 20/20 = 100% | 20/20 = 100% | 持平 |
  | 平均耗时 | 46.3s/题 | **39.6s/题** | **-14%** |
  | 城市Top5 | ✗ 10轮/126.2s | **✓ 1次成功/18.6s** | **修复，耗时-85%** |
  | 周末工作日 | ✗ 10轮/151.2s | **✓ 1次成功/58.7s** | **修复，耗时-61%** |

  - **核心验证**：路由把城市Top5/周末工作日判为 SIMPLE、不反思 → 避开了 Critic 过度纠偏拖超轮数的坑，
    这两题从**撞满10轮失败变为1次成功**。COMPLEX 档（硬口径题）正确性无回退。
  - 出图率 11/20，平均 SQL 尝试 1.30 次，与 phase-5 基线持平

  **v2 20 题**：完成率 18/20=90%，SQL自愈后100%（2 题失败：1 模型推理超时180s、1 SQL连续失败6次）
  **AB 6 题**：有效完成 4/4=100%（后 2 题触发百炼 **403 额度耗尽**——第 44 题打到上限，符合预期）

  **三组合计（排除百炼403的有效统计）**：完成率 42/44=95%，总平均耗时 37.5s/题
  （明细 eval/results-20260709-140918.json / results-20260709-142312.json / results-20260709-142450.json）

- [x] **换模型后再复跑（qwen3.7-plus 额度耗尽 → qwen3.6-flash）**（2026-07-10，同三个评测集 46 题，路由 ON + 收紧后 Critic）：
  flash 是轻量多模态模型，实测**全面优于 plus**——这印证了 phase-5 的观察（plus 迭代啰嗦拖慢延迟）：
  ...
  **原 20 题跨模型对比（苹果对苹果）：**
  ...
  | 任务完成率 | 100% | **100%** |
  | SQL 一次成功率 | 85% | **100%** |
  | 平均耗时 | 39.6s/题 | **12.9s/题（-67%）** |
  | 城市Top5 / 周末工作日 | ✓ 18.6s / 58.7s | **✓ 9.9s / 28.9s** |

  - **v2 20 题**：完成率 19/20=95%（plus 是 18/20）、SQL 一次成功率 100%、平均 18.9s（plus 41.7s）。
    plus 上失败的两题在 flash 下：安全库存补货题 ✓ 自愈成功（plus 曾 SQL 连续失败）；
    唯一失败仍是"最近90天各品牌销售额趋势"（SQL×7 达上限，多维分组查询对模型确实难，但已非超时）
  - **AB 6 题**：6/6=100%（未触发 403——flash 调用更轻、额度足；plus 曾在此耗尽额度）
  - **flash 三组合计：完成率 45/46=97%、SQL 一次成功率 46/46=100%、总平均 15.8s/题**（plus 37.5s）
  - 工程结论：轻量 flash 模型在本项目 Text-to-SQL 场景**又快又准**，是比 plus 更合适的默认底座；
